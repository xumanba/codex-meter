...
- 点击重置倒计时打开历史面板：默认显示最近 3 次和平均间隔，可展开到最近 10 次；程序直接检测标为“检测”，本机旧日志回推标为“推算”并附置信度
...
- 额度只调用本机 `codexbar-cli.exe`；近 7 天统计只读取 Codex rollout 的时间、模型、强度和 token 字段，不保存登录凭据或对话正文
...
重置历史位于 `%LOCALAPPDATA%\CodexMeter\reset-history.json`。程序只长期保存检测到的重置事件、置信度和日志读取位置，不复制对话正文。首次运行会在后台只读扫描现有 rollout 的结构化 `rate_limits` 元数据；不同任务返回值冲突或证据不足时不会冒充准确记录，低置信度记录也不参与平均间隔。
