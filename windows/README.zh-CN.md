...
- 当前预计重置时间采用不提前归零的紧凑倒计时；鼠标悬停可查看换算后的本机日期和时间
- 自适应刷新：普通显示约 60 秒、贴边/隐藏约 120 秒，失败时自动退避；同步期间再次点击“实时”会排队一次手动同步
- 深色/浅色玻璃主题
- 中等深度蓝灰冰晶用量柱图标用于 EXE、窗口和托盘；原深色及浅色源图均保留在仓库中
- Windows 卡片默认持续显示；可最小化到托盘或拖到左右边缘自动隐藏
- 默认始终置顶；勾选“取消始终置顶”后改为普通窗口层级，其他程序可以覆盖卡片。即使已取消全局置顶，当 Codex/ChatGPT 在同一显示器前台时，卡片仍会临时抬高，离开后恢复普通层级
- 菜单可直接勾选“开机自启动”，状态与当前 Windows 用户启动项同步；登录启动时，已贴边自动隐藏的卡片会先展开 30 秒，再恢复正常贴边隐藏
- 拖动位置记忆、左右贴边自动隐藏和鼠标触边唤出
- 顶部“实时”状态胶囊可直接点击立即同步；同步期间名称仍保持“实时”，蓝色状态点表示查询中，悬停显示“正在同步数据，请稍候…”；当前查询尚未结束时的重复点击会合并为一次排队重试
...
- 额度调用 ZIP 内置并经过 SHA-256 校验的 Win-CodexBar CLI 0.45.2；近 7 天统计只读取 Codex rollout 的时间、模型、强度和 token 字段，不保存登录凭据或对话正文

## 运行要求

- Windows 10/11
- Windows 自带的 .NET Framework 4.7.2 或更高版本
- 已登录 Codex 桌面客户端，或已经运行 `codex login`
- 不需要另外安装 Win-CodexBar；请完整解压 ZIP，保留同目录的 `codexbar-cli.exe`

程序按以下顺序寻找 CLI：

1. 用户显式设置的 `CODEXBAR_CLI` 环境变量；
2. 与 `CodexMeter.exe` 同目录且 SHA-256 匹配的内置 `codexbar-cli.exe`；
3. `%LOCALAPPDATA%\Programs\CodexBar\codexbar-cli.exe`；
4. `%LOCALAPPDATA%\Programs\Win-CodexBar\codexbar-cli.exe`。
...
重置历史位于 `%LOCALAPPDATA%\CodexMeter\reset-history.json`。程序只长期保存检测到的重置事件、来源、时间可信度和日志读取位置，不复制对话正文。首次运行会在后台只读扫描现有 rollout 的结构化 `rate_limits` 元数据；程序持续运行时每 30 分钟低频增量补扫一次，历史合并和写盘均不占用界面线程。不同任务返回值冲突或证据不足时不会冒充准确记录，低置信度记录也不参与平均间隔。

两份 JSON 缓存均采用临时文件原子替换，并在同目录保留上一代 `.bak` 备份。主文件损坏时会优先尝试恢复备份。经过脱敏的有限诊断日志位于 `%LOCALAPPDATA%\CodexMeter\codexmeter.log`，达到 1 MB 后轮换为一份 `.old` 文件。

Windows 模块和数据流说明见 [`ARCHITECTURE.md`](ARCHITECTURE.md)。
主卡片绘制、窗口/贴边状态和重置历史导航均已拆成可独立测试的模块；窗体保留托盘、菜单和事件编排，便于后续功能在不改变现有外观与设置的前提下扩展。
...
- Windows 版直接按分钟调用内置并校验的 `codexbar-cli.exe usage ... --format json`，不常驻占用 18747 端口，也不要求安装 Win-CodexBar 桌面程序。
...
- 缺少重置时间时不会虚构倒计时或节奏预测。
- 每周额度显示 `xd xh 后重置`；鼠标悬停可查看上游当前预计时间换算后的本机日期与时刻，点击可查看重置历史。Spark 附加窗口仍可被解析，但 Windows 界面不再显示该整栏。
