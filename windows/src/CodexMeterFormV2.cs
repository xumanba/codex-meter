...
namespace CodexMeter
{
    internal sealed class CodexMeterFormV2 : Form
    {
        ...
        private void BuildMenus()
        {
            ...
            ToolStripMenuItem openFolder = new ToolStripMenuItem("打开 Codex 会话目录");
            openFolder.Click += delegate { OpenCodexSessionsFolder(); };
        ...
        private void ResizeForContent()
        {
            ...
            bool hasLocalUsageDetails = HasLocalUsageDetails;
            if (!hasWeeklyPace && !hasLocalUsageDetails)
                detailsExpanded = false;

            int target = ContentHeight(hasSnapshot, hasWeeklyPace, detailsExpanded,
                hasLocalUsageDetails);
        ...
        internal static int ContentHeight(bool hasSnapshot, bool hasWeeklyPace,
            bool detailsExpanded, bool hasLocalUsageDetails = false)
        {
            if (!hasSnapshot)
            {
                if (!hasLocalUsageDetails)
                    return 126;
                return HeaderHeight + 54 + DailyUsageHeight + ModelHeaderHeight +
                    (MaximumModelRows * ModelRowHeight) + BottomPadding;
            }
        ...
        protected override void OnPaint(PaintEventArgs eventArgs)
        {
            ...
            if (snapshot == null)
            {
                ...
                {
                    int localY = 112;
                    DrawDailyUsage(graphics, localY);
                    localY += DailyUsageHeight;
                    DrawModelUsage(graphics, localY);
                }
        ...
        private void DrawEmptyState(Graphics graphics)
        {
            string message = String.IsNullOrWhiteSpace(lastError)
                ? "正在连接 Codex…"
                : (HasLocalUsageDetails ? "额度需 Codex CLI，本地统计可用" : lastError);
        ...
        private void DrawModelUsage(Graphics graphics, int y)
        {
            ...
            string recommendation = FavoriteModelRecommendation(weeklyUsage);
            if (!String.IsNullOrEmpty(recommendation))
            {
                RectangleF recommendationBounds = new RectangleF(147, y + 2, 161, 21);
                using (Font recommendationFont = FittedPixelFont(
                    graphics, recommendation, recommendationBounds, 10.5f, 8.5f, FontStyle.Bold))
                using (Brush recommendationBrush = new SolidBrush(IsDark
                    ? Color.FromArgb(112, 220, 255)
                    : Color.FromArgb(0, 116, 192)))
                {
                    DrawText(graphics, recommendation, recommendationFont, recommendationBrush,
                        recommendationBounds, StringAlignment.Far, StringAlignment.Center);
                }
            }
        ...
        internal static string FavoriteModelRecommendation(WeeklyTokenReport report)
        {
            if (report == null || report.FavoriteModel == null || report.TotalTokens <= 0)
                return String.Empty;

            string label = ModelLabel(report.FavoriteModel);
            if (String.IsNullOrWhiteSpace(label))
                return String.Empty;

            double share = report.FavoriteModel.Tokens * 100d / report.TotalTokens;
            return "推荐 " + label + " · " + share.ToString("0") + "%";
        }
        ...
        {
            if ((snapshot == null || snapshot.WeeklyPace == null) && !HasLocalUsageDetails)
                return;

            if (snapshot == null && HasLocalUsageDetails)
                return;
        ...
        private void UpdateAccessibleSummary()
        {
            ...
            string usage = main == null
                ? (HasLocalUsageDetails ? "额度数据尚未载入，本地统计可用" : "用量数据尚未载入")
        ...
        private void OpenCodexSessionsFolder()
        {
            ...
            {
                MessageBox.Show(this, "未找到 Codex 会话目录。请先运行 Codex 并完成至少一次会话。", "CodexMeter",
        ...
        private string BuildTrayText()
        {
            UsageWindow main = snapshot == null ? null : snapshot.Weekly ?? snapshot.Session;
            if (main == null)
                return Shorten("CodexMeter - " +
                    (HasLocalUsageDetails ? "本地统计可用" : (lastError ?? "正在同步")), 63);
        ...
        private bool HasLocalUsageDetails
        {
            get
            {
                return weeklyUsage != null &&
                    (weeklyUsage.TotalTokens > 0 ||
                     (weeklyUsage.Models != null && weeklyUsage.Models.Any(item => item != null && item.Tokens > 0)) ||
                     (weeklyUsage.Days != null && weeklyUsage.Days.Any(item => item != null && item.Tokens > 0)));
            }
        }
        ...
        private Color StatusDotColor
        {
            get
            {
                ...
                if (HasLocalUsageDetails)
                    return Color.FromArgb(53, 207, 235);
