    ...
    internal static class UsageSnapshotDecoder
    {
        ...
        public static UsageSnapshot Decode(string json)
        {
            if (String.IsNullOrWhiteSpace(json))
                throw new InvalidOperationException("Codex 返回了空数据。");
            ...
            if (usage == null)
                usage = AsDictionary(Get(payload, "rateLimits"));
            if (usage == null)
                usage = AsDictionary(Get(payload, "rate_limits"));
            if (usage == null)
                throw new InvalidOperationException("Codex 用量响应中缺少 usage/rateLimits 数据。");
        ...
        private static UsageWindow DecodeWindow(IDictionary<string, object> value, string title)
        {
            ...
            if (!window.ResetsAt.HasValue)
                window.ResetsAt = ParseUnixDate(GetEither(value, "resets_at", "resetsAt"));
            object minutes = GetEither(value, "window_minutes", "windowMinutes");
            if (minutes == null)
                minutes = GetEither(value, "window_duration_mins", "windowDurationMins");
            window.WindowMinutes = AsNullableInt(minutes);
        ...
        private static IDictionary<string, object> FirstPayload(object root)
        {
            ...
            throw new InvalidOperationException("Codex 用量响应为空。");
        }
        ...
        private static DateTimeOffset? ParseUnixDate(object value)
        {
            if (value == null)
                return null;

            double seconds;
            if (!Double.TryParse(AsString(value), NumberStyles.Float, CultureInfo.InvariantCulture, out seconds))
                return null;

            if (seconds <= 0 || seconds > 253402300799d)
                return null;

            return DateTimeOffset.FromUnixTimeSeconds(Convert.ToInt64(Math.Floor(seconds)));
        }
