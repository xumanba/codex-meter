...
2. Codex CLI 是可选依赖。未安装 Codex CLI 时，CodexMeter 仍可显示本机
   近 7 天 token、模型偏好和最喜欢的模型推荐；每周额度剩余、重置时间
   和节奏预测会显示“额度需 Codex CLI，本地统计可用”。
   安装并登录 Codex CLI 后，CodexMeter 会通过 `codex app-server --stdio`
   自动读取本机 Codex 额度。如果 Codex CLI 不在 PATH 中，可设置
   CODEX_CLI 环境变量指向 codex.exe 或 codex.cmd。
...
9. 鼠标悬停在“实时”状态上，可通过与额度进度条同色系的圆角提示
   查看最近的数据更新时间；无 CLI 降级模式下状态会显示“本地”。
...
13. 新增本机近 7 天 token、每日额度折算、模型/推理强度偏好和最喜欢的
    模型推荐。增量缓存不保存提示词、回复或凭据；缺少模型字段时显示
    “未标注模型”。
...
2. Codex CLI is optional. Without Codex CLI, CodexMeter still shows local
   seven-day tokens, model preferences and a favorite-model recommendation.
   Weekly allowance, reset time and pace forecasting show a local-only message.
   After Codex CLI is installed and signed in, CodexMeter reads local allowance
   data through `codex app-server --stdio`. Win-CodexBar and codexbar-cli.exe are
   no longer required. If Codex CLI is not on PATH, set CODEX_CLI to codex.exe
   or codex.cmd.
...
9. Hover the status pill to see the latest data-update time in a rounded prompt
   that uses the same color palette as the allowance progress bar. In local-only
   mode, the status label reads "本地".
...
13. The card adds local seven-day tokens, daily quota shares, model/reasoning-
    effort preferences and a favorite-model recommendation. Its incremental cache
    stores no prompts, responses or credentials; missing model metadata is shown
